...
*/
	...
	function ErrorMsg() 
	{
		...
		try {
			...
			$error_count = $errc->Count-1;
			$err = $errc->Item($error_count);
		...
		for ($i=0,$max = $this->_numOfFields; $i < $max; $i++) {
			...
			case 205: // CAMPO DO BLOB
				...
				else
				{
					...
					$mstream->Open;
					$mstream->Write($f->value);
					$sess = md5(date('Ymd Hisu') . rand(0, 1000));
					$mstream->SaveToFile(ADODB_DIR . '/log/sql_adodb_streamlogs_'. $sess .'.tmp', 2);
					
					if(is_file(ADODB_DIR . '/log/sql_adodb_streamlogs_'. $sess .'.tmp'))
					{
						$val = file_get_contents(ADODB_DIR . '/log/sql_adodb_streamlogs_'. $sess .'.tmp');				
						
						@unlink(ADODB_DIR . '/log/sql_adodb_streamlogs_'. $sess .'.tmp');
					}
				}
