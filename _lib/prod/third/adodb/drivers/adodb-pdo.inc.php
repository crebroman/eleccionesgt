						...
						switch($this->dsnType)
                        {
                            ...
                            case 'sqlite':
                                if (strtoupper(substr(PHP_OS, 0, 3)) == 'WIN') {
                                    $os = 'Windows/libspatialite_x64.dll';
                                } elseif (PHP_OS == 'Darwin') {
                                    $os = 'Mac/libspatialite.dylib';
                                } else {
                                    $os = 'Linux/' . (2147483647 == PHP_INT_MAX ? "libspatialite-i386.so" : "libspatialite-amd64.so");
                                }

                                $file = dirname(__DIR__) . "/ext/spatialite/" . $os;

                                $this->_connectionID->exec('select load_extension("' . $file . '")');
